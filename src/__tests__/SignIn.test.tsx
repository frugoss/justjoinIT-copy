...
import SignIn, {SignInProps} from "../App/views/Auth/SignIn";
...
function renderSignIn(props: Partial<SignInProps> = {}) {
    ...
    return renderWithRouter(<SignIn {...defaultProps} {...props} />, {route: '/devs'});
}
test("Test if it gets redirected after successful login",  async () => {
    const {history} = renderSignIn();
    userEvent.type(screen.getByRole("textbox", {name: /email/i}), 'kowalski@gmail.com');
